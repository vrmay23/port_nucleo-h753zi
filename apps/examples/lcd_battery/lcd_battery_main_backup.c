...
static void show_usage(const char *progname)
{
  ...
  printf("\n");
  printf("Default: both flip-x and flip-y enabled (working config)\n");
}
 ...
 *
 * Description:
 *   Query display dimensions from driver using ioctl.
 *   Allocates framebuffer based on actual display size.
 *
 ****************************************************************************/
...
static int lcd_get_info(int fd)
{
  ...
  if (ret < 0)
    {
      fprintf(stderr, "ERROR: LCDDEVIO_GETVIDEOINFO failed: %d\n", errno);
      return ret;
    }
  ...
  printf("Display detected: %dx%d pixels\n", g_lcd_width, g_lcd_height);

  /* Validate dimensions */

  if (g_lcd_width > LCD_MAX_WIDTH || g_lcd_height > LCD_MAX_HEIGHT)
    {
      fprintf(stderr, "ERROR: Display too large (%dx%d > %dx%d)\n",
  ...
  if (g_lcd_width == 0 || g_lcd_height == 0)
    {
      fprintf(stderr, "ERROR: Invalid display dimensions\n");
      return -EINVAL;
    }
  ...
  if (g_framebuffer == NULL)
    {
      fprintf(stderr, "ERROR: Failed to allocate framebuffer (%zu bytes)\n",
              fb_size);
      return -ENOMEM;
    }
