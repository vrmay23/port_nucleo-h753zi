 ...
 *   - The interrupt must be explicitly enabled after attachment using the enable operation
 *   
  ...
  if (enable)
    {
      ...
      if (stm32gpint->callback == NULL)
        {
          gpioerr("ERROR: Cannot enable GPIO interrupt %d - no callback attached\n",
                  stm32gpint->stm32gpio.id);
          return -EINVAL;
        }
  ...
  for (i = 0; i < BOARD_NGPIOOUT; i++)
    {
      ...
      gpioinfo("GPIO output %d: configured as LOW and registered as /dev/gpio%d\n", 
               i, (*pincount) - 1);
    }
